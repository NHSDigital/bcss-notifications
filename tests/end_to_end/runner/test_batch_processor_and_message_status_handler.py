...
def send_all_status_callbacks(cursor, recipient_data):
    ...
    assert len(results) == len(
        recipient_data
    ), "Not all messages were marked as sending"
...
def post_callback(post_body):
    ...
    headers = {
        ...
        "x-hmac-sha256-signature": create_digest(
            signature_secret, json.dumps(post_body, sort_keys=True)
        ),
    }
    ...
    response = requests.post(
        client_endpoint, headers=headers, json=aws_lambda_post_body
    )
    logging.debug("Response from callback: %s: %s", response.status_code, response.text)

    if response.status_code != 200:
        pytest.fail(
            f"Failed to post callback: {response.status_code} - {response.text}"
        )
        ...
        "data": [
            {
                ...
                "meta": {"idempotencyKey": uid(64)},  # gitleaks:allow
            }
        ]
    }
        ...
        "data": [
            {
                ...
                "attributes": {
                    ...
                    "channels": [{"type": "email", "channelStatus": channel_status}],
                    "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S.%fZ"),
                    "routingPlan": {
